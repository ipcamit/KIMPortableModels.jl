...
@testset "High-level Interface Integration Tests" begin
    ...
    @testset "KIMModel Function Creation" begin
        try
            ...
            custom_units = (kim_api.A, kim_api.eV, kim_api.e, kim_api.K, kim_api.ps)
            ...
            @test_throws ErrorException kim_api.KIMModel(test_model_name, compute=[:invalid])  # Invalid property
    ...
    @testset "Basic Computation" begin
        try
            ...
            a = 5.43  # Angstroms
            cell = a * I(3)  # 3x3 identity matrix scaled by lattice parameter
            pbc = [true, true, true]
            ...
            if contains(string(e), "Model creation failed") || contains(string(e), "not found")
                @test_skip "Test model $test_model_name not available"
            else
                rethrow(e)  
            end
    ...
    @testset "Error Handling" begin
        try
            ...
            @test_throws ErrorException model(["Cu"], [SVector(0.0, 0.0, 0.0)], I(3), [true, true, true])
            
            # Test with mismatched species/positions lengths
            @test_throws BoundsError model(["Si"], [SVector(0.0, 0.0, 0.0), SVector(1.0, 0.0, 0.0)], I(3), [true, true, true])
            
            # Test with invalid cell (non-3x3 matrix)
            invalid_cell = [1.0 0.0; 0.0 1.0]  # 2x2 matrix
            @test_throws Exception model(["Si"], [SVector(0.0, 0.0, 0.0)], invalid_cell, [true, true, true])
