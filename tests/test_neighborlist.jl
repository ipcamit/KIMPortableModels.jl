...
using Random
using StaticArrays
using KIM  # Assuming your module is named KIM
...
@testset "KIM Neighbors Tests" begin
    ...
    @testset "NeighborListContainer" begin
        ...
        dummy_fn = x -> [1, 2, 3]
        container = KIM.NeighborListContainer(dummy_fn)
    ...
    @testset "create_neighborlist_closure" begin
        ...
        get_neighbors = KIM.create_neighborlist_closure(positions, cutoff, cell, pbc)
        ...
        @test_throws ArgumentError KIM.create_neighborlist_closure(positions, cutoff, nothing, [true, true, true])
    end
    ...
    @testset "create_kim_neighborlist_dataobject" begin
        ...
        nl_containers = KIM.create_kim_neighborlist_dataobject(positions, cutoffs, cell)
        
        @test length(nl_containers) == 3
        @test all(c isa KIM.NeighborListContainer for c in nl_containers)
    end
    ...
    @testset "kim_neighbors_function" begin
        ...
        nl_containers = KIM.create_kim_neighborlist_dataobject(positions, cutoffs, cell, [false, false, false])
        ...
        result = KIM.kim_neighbors_function(
            dataObject,
    ...
    @testset "Index conversion" begin
        ...
        positions = [SVector(float(i), 0.0, 0.0) for i in 0:4]
        cutoff = 1.5
        cell = [10.0 0.0 0.0; 0.0 10.0 0.0; 0.0 0.0 10.0]
        
        get_neighbors = KIM.create_neighborlist_closure(positions, cutoff, cell, [false, false, false])
        ...
        nl_containers = [KIM.NeighborListContainer(get_neighbors)]
        dataObject = pointer_from_objref(nl_containers)
    ...
    @testset "Large system performance" begin
        ...
        Random.seed!(42)
        n_atoms = 1000
        positions = [SVector(10*rand(), 10*rand(), 10*rand()) for _ in 1:n_atoms]
        cutoff = 1.0
        cell = [10.0 0.0 0.0; 0.0 10.0 0.0; 0.0 0.0 10.0]
        
        nl_containers = KIM.create_kim_neighborlist_dataobject(positions, [cutoff], cell, [true, true, true])
